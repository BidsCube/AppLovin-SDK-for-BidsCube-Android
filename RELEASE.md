...
| Item | Source | Example (1.2.14) |
|------|--------|-----------------|
| **SDK Maven version** | `BidscubeVersion` env or `sdk/build.gradle.kts` default | `1.2.14` |
| **Adapter Maven version** | `BidscubeAdapterVersion` env or `applovin-adapter/build.gradle.kts` default | `1.2.14` |
| **Runtime SDK version** | `BuildConfig.SDK_VERSION_NAME` / `BidscubeSDK.getSdkVersion()` | `1.2.14` |
| **Runtime adapter version** | `BidscubeMediationAdapter.getAdapterVersion()` fallback string | `1.2.14` |
| **Git tag (full stack)** | `v` + version | `v1.2.14` |
| **Git tag (adapter only)** | `applovin-adapter-v` + version | `applovin-adapter-v1.2.14` |
...
```bash
git tag -a v1.2.14 -m "Bidscube Android SDK + AppLovin MAX adapter 1.2.14"
...
```bash
git tag -a applovin-adapter-v1.2.14 -m "AppLovin Bidscube MAX adapter 1.2.14"
git push origin applovin-adapter-v1.2.14
```

## Maven coordinates (1.2.14)
...
This release aligns with the Unity AppLovin MAX package bundled native AARs at **1.2.14** (adapter) and core SDK **1.2.5+** four-mode export. See [AppLovin-SDK-for-BidsCube-Unity](https://github.com/BidsCube/AppLovin-SDK-for-BidsCube-Unity).
