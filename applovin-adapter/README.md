...
**Adapter version 1.2.9** · Maven group `com.bidscube`
...
- **One** Bidscube adapter artifact at **1.2.9** for your video mode (see [Android AAR modes](#android-aar-modes))
...
# applovin-adapter/build/staged-aars/applovin-bidscube-max-adapter-*-1.2.9.aar
```
...
dependencies {
    implementation 'com.applovin:applovin-sdk:13.0.0@aar'
    implementation files('libs/applovin-bidscube-max-adapter-full-video-1.2.9.aar')
}
```
...
- If reward never fires, confirm you are on **1.2.9+** — reward is only forwarded from `onUserRewarded`.
- **Show / request failures:** see [Error codes](../docs/errors.md) — e.g. code **204** = SSP no fill, **1004** = missing Activity (fixed in 1.2.9+ with official adapter).

## Release

- **Artifacts:** `com.bidscube:applovin-bidscube-max-adapter-*` (four variants at **1.2.9**)
- **Version source:** `BidscubeAdapterVersion` env var, default `1.2.9`
