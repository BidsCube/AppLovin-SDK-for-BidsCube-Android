...
**Adapter version 1.2.8** · Maven group `com.bidscube`
...
- **One** Bidscube adapter artifact at **1.2.8** for your video mode (see [Android AAR modes](#android-aar-modes))
...
|------|----------------|---------------|
| **LiteNoVideo** | `com.bidscube:applovin-bidscube-max-adapter-lite-no-video:1.2.8` | None (image/banner/native only) |
| **WebViewVideoNoDesugar** | `com.bidscube:applovin-bidscube-max-adapter-webview-video:1.2.8` | WebView / HTML5 |
| **LegacyMediaVideoNoDesugar** | `com.bidscube:applovin-bidscube-max-adapter-legacy-media-video:1.2.8` | VideoView / MediaPlayer |
| **FullWithVideo** | `com.bidscube:applovin-bidscube-max-adapter-full-video:1.2.8` | Media3 / Google IMA |
...
dependencies {
    ...
    implementation 'com.bidscube:applovin-bidscube-max-adapter-full-video:1.2.8@aar'
}
```
...
```bash
export BidscubeVersion=1.2.8
export BidscubeAdapterVersion=1.2.8
./gradlew :applovin-adapter:stageReleaseAars -PskipSigning=true
# applovin-adapter/build/staged-aars/applovin-bidscube-max-adapter-*-1.2.8.aar
```
...
dependencies {
    implementation 'com.applovin:applovin-sdk:13.0.0@aar'
    implementation files('libs/applovin-bidscube-max-adapter-full-video-1.2.8.aar')
}
```
...
- If reward never fires, confirm you are on **1.2.8+** — reward is only forwarded from `onUserRewarded`.
- **Show / request failures:** see [Error codes](../docs/errors.md) — e.g. code **204** = SSP no fill, **1004** = missing Activity (fixed in 1.2.8+ with official adapter).

## Release

- **Artifacts:** `com.bidscube:applovin-bidscube-max-adapter-*` (four variants at **1.2.8**)
- **Version source:** `BidscubeAdapterVersion` env var, default `1.2.8`
