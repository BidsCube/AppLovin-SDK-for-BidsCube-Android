...
**SDK 1.2.13** · **AppLovin MAX adapter 1.2.13**
...
- **Adapter:** one of `com.bidscube:applovin-bidscube-max-adapter-*` at **1.2.13** (see [Android AAR modes](#android-aar-modes))
...
| Native | **Not supported** via MAX adapter in 1.2.13 | — |
...
Pick **one** standalone SDK artifact at **1.2.13** (override with env `BidscubeVersion` when building from source):

```kotlin
dependencies {
    implementation("com.bidscube:sdk-full-video:1.2.13@aar")
}
```
...
SDKConfig config = new SDKConfig.Builder(this)
        ...
        .autoClose(false)  // default: keep fullscreen video open after linear playback
        .userId(publisherUserId)  // optional; sent as user_id on /sdk requests
        // Optional: .adRequestAuthority("example.com")
        .build();

BidscubeSDK.initialize(this, config);
// Or after login: BidscubeSDK.setUserId(publisherUserId);
```
...
| **`userId`** | Optional publisher user id → query param **`user_id`** |
| **`autoClose`** | Optional; default **`false`**. When `false`, fullscreen video stays open after linear playback (post-roll mini-game / interactive end card); user closes manually. Set `true` for legacy auto-dismiss. |
...
- Build all SDK AARs: `./gradlew :sdk:stageReleaseAars` → `sdk/build/staged-aars/bidscube-sdk-*-1.2.13.aar`
- Build all adapter AARs: `./gradlew :applovin-adapter:stageReleaseAars` → `applovin-adapter/build/staged-aars/applovin-bidscube-max-adapter-*-1.2.13.aar`
...
**Gradle dependency examples** (version **1.2.13**):

```kotlin
// LiteNoVideo — no video; rewarded/interstitial video return unsupported / no fill
implementation("com.bidscube:applovin-bidscube-max-adapter-lite-no-video:1.2.13@aar")

// WebViewVideoNoDesugar — HTML5 video via WebView
implementation("com.bidscube:applovin-bidscube-max-adapter-webview-video:1.2.13@aar")

// LegacyMediaVideoNoDesugar — legacy Android media player path
implementation("com.bidscube:applovin-bidscube-max-adapter-legacy-media-video:1.2.13@aar")
